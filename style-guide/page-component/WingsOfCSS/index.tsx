...
const WingsOfCSS = () => {
  ...
  const page:HTMLElement|null= document.querySelector(".wings_wings_page__VNLh_");
  const tooltip:HTMLElement|null= document.querySelector(".wings_tooltip__7E7lR");

  const handleChange=(e:Event|any) => {
    const scrollVal=e.target.value;
    if(page !== null){
      page.scrollTop = scrollVal*70;
    }
    if(tooltip !== null){
  ...
  useEffect(()=>{
    const output:HTMLElement|null= document.querySelector(".wings_wings_page__VNLh_");
    if(output !== null){
      output.addEventListener("scroll",()=>{
        setScrollDiv((output.scrollTop)/80);
        const tooltip:HTMLElement|any= document.querySelector(".wings_tooltip__7E7lR");
        setTooltipData(data.data[Math.round(output.scrollTop/750)].tooltipName);
  ...
  return (
    <div className={Styles.wings_page}>
        ...
        {data.data.map((element)=>{
          return(
            <WingCard key={element.wingName} wingName={element.wingName} description={element.description} tooltipName={element.tooltipName} />
          )
        })}
        <div className={Styles.wings_scrollBar}>
          <input type="range" name="scroll" id="scroll" className={Styles.scroll} min="0" max="60" step="10"  value={scrollDiv} onChange={handleChange} />
